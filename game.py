...
class Game:
    ...
    def update(self, dt):
        ...
        if self.selected_phase == 1 and not self.boss_spawned:
            ...
            if elapsed >= 10000 and self.player.hp > 0:
                self.spawn_boss()
        elif self.selected_phase == 3:
            if not self.miniboss_spawned:
                # Spawn miniboss slightly after phase starts
                elapsed = pygame.time.get_ticks() - self.phase_start_time
                if elapsed >= 1000 and self.player.hp > 0:
                    self.spawn_miniboss()
                ...
                elapsed = pygame.time.get_ticks() - self.phase_start_time
                if elapsed >= 3000 and self.player.hp > 0:
                    self.spawn_boss()
        ...
        # 3. Enemy Attack Hitbox vs Player Hurtbox
        if self.player.hp > 0 and self.player.hurt_timer <= 0:
            for hb in list(self.enemy_attacks):
                if id(self.player) in hb.already_hit_targets:
                    continue
                # Phase 1 – Vertical depth filter (melee)
                if math.fabs(hb.owner.foot_y - self.player.foot_y) > 50:
                    continue
                # Phase 2 – 2D AABB collision
                if hb.rect.colliderect(self.player.hurtbox):
                    hb.already_hit_targets.add(id(self.player))
                    old_hp = self.player.hp
                    self.player.take_damage(hb.damage, source_x=hb.owner.rect.centerx)
                    if self.player.hp < old_hp:
                        dmg_num = DamageNumber(self.player.hurtbox.centerx, self.player.hurtbox.top, hb.damage, is_crit=False)
                        self.damage_numbers.add(dmg_num)
                        
                        vfx_type = "death" if self.player.hp <= 0 else "hit"
                        blood = BloodVFX(self.player.hurtbox.centerx, self.player.hurtbox.centery, getattr(self.player, 'facing', 1), self.player.foot_y, vfx_type=vfx_type)
                        self.effects.add(blood)
                        hit_effect = HitVFX(self.player.hurtbox.centerx, self.player.hurtbox.centery, getattr(self.player, 'facing', 1), self.player.foot_y)
                        self.effects.add(hit_effect)
                    break

        # 4. Enemy Projectile vs Player Hurtbox
        if self.player.hp > 0 and self.player.hurt_timer <= 0:
            for proj in list(self.enemy_projectiles):
                # Phase 1 – Vertical depth filter (projectile)
                if math.fabs(proj.floor_y - self.player.foot_y) > 50:
                    continue
                # Phase 2 – 2D AABB collision
                if proj.rect.colliderect(self.player.hurtbox):
                    old_hp = self.player.hp
                    self.player.take_damage(proj.damage, source_x=proj.rect.centerx)
                    if self.player.hp < old_hp:
                        dmg_num = DamageNumber(self.player.hurtbox.centerx, self.player.hurtbox.top, proj.damage, is_crit=False)
                        self.damage_numbers.add(dmg_num)
    ...
    def draw(self):
        ...
        if isinstance(self.player, Knight) and self.player.hp > 0:
    ...
    def draw_knight_ultimate_hud(self):
        ...
        cd = max(0.0, getattr(self.player, 'ultimate_cooldown', 0))
        ready = cd <= 0
